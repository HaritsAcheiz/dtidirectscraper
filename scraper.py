...
@dataclass
class FTScraper:
	base_url: str = 'https://dtidirect.com'
	...
	def get_product_count(self, url):
		...
		product_count = int(tree.css_first('span.collection__showing-count').text(strip=True).split()[-2])
	...
	def get_data(self):
		...
		conn = duckdb.connect("dtidirect.db")
		curr = conn.cursor()
		...
		for data in datas:
			...
			for script in script_tags:
				if 'window.essentialCountdownTimerMeta' in script.text():
	...
	def fetch_search_result_html(self, url):
		...
		self.insert_to_db(search_results_html, database_name='dtidirect.db', table_name='search_src')
	...
	def get_product_urls(self):
		...
		for data in datas:
			tree = HTMLParser(data[1])
			product_elems = tree.css('a.product-item__title')
			product_urls = list()
	...
	def fetch_product_html(self, urls):
		...
		self.insert_to_db(product_htmls, database_name='dtidirect.db', table_name='product_src')
